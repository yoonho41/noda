      ...
      const message = await Promise.all(
        res.map(async (m) => {
          ...
          return {
            ...
            fromEmail: m.sender_id,
            receiver_id: m.receiver_id,
            receiver_name: Array.isArray(name) ? name[0] : name, // 배열 대응
            toEmail: m.receiver_id,
            subject: m.subject,
            content: m.content,
            send_date: m.send_date,
          ...
          return (
            ...
            >
              ...
              <div className={styles.colFrom}>
                ...
                {
                  isReceiveMessage ? 
                  <div className={styles.sender_id}> {(m.fromEmail===myEmail)?<>나</>:<>{m.sender_id}</>}</div> : 
                  <div className={styles.sender_id}>{(m.toEmail===myEmail)?<>나</>:<>{m.receiver_name}</>}</div>
                }
              ...
              <div className={styles.colActions} onClick={(e) => e.stopPropagation()}>
                {
                  ...
                  :
                    <button className={styles.linkBtn} type="button" onClick={() => openReply(m)} disabled>답장</button>
                }
                {/* <button className={styles.linkBtn} type="button" onClick={() => openReply(m)} >답장</button> */}
                ...
                }}>삭제</button>
                {
                  ...
                  :
                    <button className={styles.linkBtn} type="button" onClick={() => onUnRead(m.id)} disabled>안읽음</button>
                }
